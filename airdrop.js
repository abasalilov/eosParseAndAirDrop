...
const filterLists = async snapshot => {
  ...
  const filteredSnapshot =
    blacklist && blacklist.accounts && blacklist.accounts.length > 0
      ? snapshot.filter(tuple => {
          if (blacklist.accounts.indexOf(tuple.account) < 0) {
            return true;
          } else {
            logger.warn(
              `Account ${tuple.account} was blacklisted - Amount: ${
                tuple.amount
              }`
            );
            return false;
  ...
  if (
    filteredSnapshot.length != snapshot.length &&
  ...
  if (config.limitCap && config.limitCap > 0) {
    const cappedSnapshot = filteredSnapshot.map(tuple => {
      ...
      if (!isWhite && amount > config.limitCap) {
        ...
        logger.warn(
          `Account ${tuple.account} was capped - it had ${
            tuple.amount
          } before being capped to ${amount}`
        );
      } else if (isWhite) {
        logger.warn(
          `Account ${
            tuple.account
          } is whitelist, therefore not capped - it has a total amount of ${
            tuple.amount
          }`
        );
      }
    ...
    if (filteredSnapshot.length != snapshot.length) {
      logger.warn(
        `You are limitting token holders to receive a max airdrop corresponding to ${
          config.limitCap
        } EOS`
      );
...
const run = async () => {
  const getRatio = tuple =>
    (tuple.amount * config.ratio).toFixed(config.decimals);
  ...
  if (!(await EOSTools.fillTokenStats(config))) {
    logger.error(
      `\r\nCould not find ${
        config.symbol
      } token on the eosio.token contract at ${config.tokenAccount}!`
    );
    process.exit();
  }
  ...
  const ratioBalances = accountBalances.map(tuple =>
    Object.assign(tuple, { amount: 1000 })
  );

  const ram = await EOSTools.estimateRAM(accountBalances, config);
  if (
    (await Prompter.prompt(
      `\r\nThis airdrop will require that ${
        config.issuer
      } has an estimated minimum of ${ram[0]}KB of RAM, at the cost of ${
        ram[1]
      } at the current price of ${ram[2]}. \r\nPress enter to continue`
    )) !== ""
  )
    process.exit();

  const total = ratioBalances
    .reduce((acc, e) => (acc += parseFloat(e.amount)), 0)
    .toFixed(config.decimals);

  if (
    (await Prompter.prompt(
      `\r\nYou are about to airdrop ${total} ${config.symbol} tokens on ${
  ...
  logger.warn(
    `Starting to airdrop from account '${
      lastAccountDropped.length ? lastAccountDropped : ratioBalances[0].account
    }'`
  );
  logger.warn(
    "\r\n------------------------------------------------------------------\r\n"
  ...
  console.log("ration", ratioBalances);
