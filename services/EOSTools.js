...
exports.setNetwork = async network => {
  ...
  await Eos({ httpEndpoint: network })
    .getInfo({})
    .catch(() => {
...
exports.fillTokenStats = async config => {
  ...
  return await eos
    ...
    .catch(() => {
      logger.error(
        `ERROR: Could not get token info from account: '${
          config.tokenAccount
        }' for the symbol '${config.symbol}'`
      );
      process.exit();
    });
};
...
exports.estimateRAM = async (accountBalances, config) => {
  ...
  const getRamInfo = async () =>
    (await getEos())
      ...
      .then(res => {
        ...
        return [
          parseAsset(ramInfo.quote.balance),
          parseAsset(ramInfo.base.balance),
          systemSymbol
        ];
      });

  const fetchEOSPrice = async () => {
    return (await fetch("https://api.coinmarketcap.com/v1/ticker/eos/").then(
  ...
  const totalKB = (
    accountBalances
      .map(tuple => `${tuple.account}${tuple.amount} ${config.symbol}`)
      .join("").length / 1024
  ).toFixed(8);
  ...
  return await eos
    ...
    .then(res => {
      return {
        ...
        dropped: !!res.rows.filter(row => row.balance.split(" ")[1] === symbol)
          .length
      };
    })
    .catch(err => {
      logger.warn(
        `ERROR: Failed to get tokens for ${
          tuple.account
        }. Considering this account already dropped and continuing. - `,
        err
      );
...
exports.dropTokens = async (accountBalances, config) => {
  ...
  const startingIndex = lastAccountDropped.length
    ? accountBalances.findIndex(e => e.account === lastAccountDropped)
    : 0;
  const accountsFrom = accountBalances.slice(
    startingIndex,
  ...
  if (startingIndex > 0)
    logger.warn(
      `Dropping to ${
        accountsFrom.length
      } accounts, already processed ${accountBalances.length -
        accountsFrom.length} accounts`
    );
...
const recurseBatch = async (accountBalances, eos, contract, auth, config) => {
  return new Promise(async resolve => {
    ...
    const getBatch = size => {
      const batch = [];
      while (batch.length < size && accountBalances.length)
        batch.push(accountBalances.shift());
      return batch;
    };
    ...
    setTimeout(
      async () =>
        await recurseBatch(accountBalances, eos, contract, auth, config),
      150
    );
  });
};
...
const dropBatch = async (batch, eos, contract, auth, config, tries = 0) => {
  ...
  const filters = await pMap(
    batch,
    tuple => getBalance(eos, tokenAccount, symbol, tuple),
    { concurrency: batchSize < 20 ? batchSize : 20 }
  );
  batch = batch.filter(tuple =>
    filters.find(filter => filter.account === tuple.account && !filter.dropped)
  );
  ...
  const dropped = await contract
    .transaction(tr =>
      batch.map(tuple => {
        console.log("tuple", tuple);
        return tr.issue(tuple.account, `${tuple.amount} ${symbol}`, memo, auth);
      })
    )
  ...
  if (!dropped) {
    ...
    logger.error(
      "You should restart the airdrop with the first account in the list above"
    );
    logger.error("\r\n-------------------------------------\r\n");
